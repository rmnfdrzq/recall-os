...
The current implementation is a client-first RAG workspace: document metadata, extracted chunks, vectors, and search index data are stored in the desktop app's local LanceDB database. The Django server is still required for chat persistence, Groq-first/server-side-Ollama-fallback LLM responses, document summary and category generation, server-side fallback parsing, and BGE-M3 embeddings.
...
Local file
  ...
  -> server /api/embeddings/ backed by server-host Ollama BGE-M3
  ...
  -> Django builds the final prompt and calls Groq first, with server-host Ollama LLM fallback
```
...
- Host/server Ollama reachable by the backend through `OLLAMA_BASE_URL`.
- An Ollama embedding model compatible with `bge-m3`.
- Groq API key configured on the backend for primary LLM calls.
- An Ollama LLM model configured by the backend for server-side fallback, currently defaulting to `gemma4:31b-cloud`.
...
- `POST /api/chat/session/<id>/message/` for final prompt construction, Groq-first generation with server-host Ollama fallback, and assistant message persistence.
...
Server fallback supports text-like files, digital PDFs, rendered PDF pages, and images. Image/screenshot/scan extraction uses the backend visual LLM route: the universal Groq model first, then server-host Ollama LLM fallback if Groq fails. There is no EasyOCR dependency in the active server requirements.
...
The backend calls the Ollama daemon reachable from `recall-server`, preferring `/api/embed` and falling back to `/api/embeddings` when needed. The client names this helper `generateServerEmbeddingsBatch`.
...
- Embeddings are generated through the server's Ollama route, not in the WebView.
- Browser-only mode cannot index documents locally.
- Image/PDF visual extraction depends on the configured backend visual LLM route. The universal Groq model is primary and server-host Ollama `gemma4:31b-cloud` is the fallback.
