...
export default function App() {
  ...
  const [modelLoadingLabel, setModelLoadingLabel] =
    useState("Preparing download");
  ...
  const initLocalEngine = useCallback(async () => {
    ...
    if (isDesktop()) {
      ...
      try {
        ...
        setModelLoadingError(
          err?.message || "Failed to download the local BGE-M3 model.",
        );
      } finally {
  ...
  const refreshLocalDocuments = async () => {
    ...
    const staleBgeFailures = localDocs.filter(
      (doc) =>
        doc.status === "failed" &&
        String(doc.summary || "").includes(
    ...
    for (const staleDoc of staleBgeFailures) {
      try {
        ...
        console.warn(
          "Failed to remove stale BGE-M3 download error document",
          err,
        );
      }
    }
    ...
    const normalized = localDocs
      .filter(
        (doc) => !staleBgeFailures.some((staleDoc) => staleDoc.id === doc.id),
      )
  ...
  const processFileOnServer = async (filePath, fileObject) => {
    console.log("🖥️ [Recall App] [Stage 1] processFileOnServer initiated:", {
      filePath,
      hasFileObject: !!fileObject,
    });
    const formData = new FormData();
    const uploadFileObject =
      fileObject || (await buildUploadFileFromPath(filePath));
    formData.append("file", uploadFileObject);

    console.log(
      `📤 [Recall App] [Stage 1] Sending POST to stateless server endpoint: ${SERVER_PROCESS_ENDPOINT}`,
    );
    ...
    try {
      ...
      if (!res.ok) {
        console.error(
          `❌ [Recall App] [Stage 1] Server OCR request failed after ${duration}ms:`,
          data.error || "Server fallback processing failed",
        );
        throw new Error(data.error || "Server fallback processing failed");
      }
      console.log(
        `📥 [Recall App] [Stage 1] Server OCR response received successfully in ${duration}ms:`,
        {
      ...
      console.error(
        `❌ [Recall App] [Stage 1] Server OCR fallback request threw exception:`,
        err,
      );
      throw err;
    }
  ...
  const normalizeIndexedChunks = (filename, text, chunks = []) => {
    ...
    return chunks
      .map((chunk, index) => ({
        ...
        chunk_index: Number.isFinite(chunk.chunk_index)
          ? chunk.chunk_index
          : index,
  ...
  const persistLocalChunks = async (documentId, filename, chunks) => {
    console.log(
      `🏎️ [Recall App] [Stage 2] persistLocalChunks called for document: ${documentId} ("${filename}") with ${chunks.length} chunks.`,
    );
    const dbChunks = [];
    const chunksToEmbed = [];
    ...
    console.log(
      "💾 [Recall App] [Stage 2] Checking IndexedDB local embeddings cache for duplicate chunks...",
    );
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      console.log(
        `🔄 [Recall App] [Stage 2] Chunk [${i}] cache check: hash = ${hash}. Found in cache? ${!!cachedVector}`,
      );
    ...
    console.log(
      `📊 [Recall App] [Stage 2] Cache search finished. Total valid chunks: ${dbChunks.length}. Cache hits (reused): ${cachedHitsCount}. Cache misses (needs embedding): ${chunksToEmbed.length}.`,
    );

    // Step 2: Compute remaining vectors in batches off main thread
    if (chunksToEmbed.length > 0) {
      console.log(
        `🛰️ [Recall App] [Stage 2] Dispatching ${chunksToEmbed.length} chunks to stateless server for batch embeddings generation using BGE-M3 model...`,
      );
      const BATCH_SIZE = 16;
      const totalBatches = Math.ceil(chunksToEmbed.length / BATCH_SIZE);
      console.log(
        `📨 [Recall App] [Stage 2] Batch processing in chunks of size ${BATCH_SIZE}. Total batches: ${totalBatches}`,
      );

      for (
        let offset = 0;
        offset < chunksToEmbed.length;
        offset += BATCH_SIZE
      ) {
        const batchTexts = chunksToEmbed.slice(offset, offset + BATCH_SIZE);
        const batchIndices = chunkIndicesToEmbed.slice(
          offset,
          offset + BATCH_SIZE,
        );
        ...
        console.log(
          `🚀 [Recall App] [Stage 2] Requesting server embeddings batch ${currentBatchNum}/${totalBatches}: offset = ${offset}, batch size = ${batchTexts.length}`,
        );
        ...
        console.log(
          `📡 [Recall App] [Stage 2] Server embeddings batch ${currentBatchNum}/${totalBatches} response received in ${duration}ms. Received ${batchVectors.length} vectors.`,
        );
      ...
      console.log(
        `💾 [Recall App] [Stage 2] All new vectors successfully computed and cached in IndexedDB.`,
      );
    } else {
      console.log(
        "♻️ [Recall App] [Stage 2] All chunks were found in cache! Zero server roundtrips required.",
      );
    }
    ...
    console.log(
      `💾 [Recall App] [Stage 2] Writing all ${formattedDbChunks.length} chunks with BGE-M3 vectors to LanceDB via Tauri invoke('insert_document_chunks')...`,
    );
    ...
    console.log(
      `✅ [Recall App] [Stage 2] LanceDB insertion completed successfully in ${Date.now() - dbStartTime}ms.`,
    );
  };
      ...
      try {
        ...
        if (sessionData.length > 0) {
          ...
          const detailRes = await fetch(
            `${API_BASE}/chat/session/${firstSessionId}/`,
          );
          if (detailRes.ok) {
            ...
            setChatMessages([
              {
                ...
                role: "assistant",
                content:
                  "Hello! I am RecallOS AI. I have analyzed your documents in the local library. Ask me any question about them!",
                sources: [],
              },
            ]);
          }
  ...
  const uploadLocalFile = async (filePath, fileObject) => {
    ...
    console.log("📂 [Recall App] [Stage 0] File upload initiated:", {
      filePath,
      filename,
      extension,
      now,
    ...
    try {
      ...
      console.log(
        "📝 [Recall App] [Stage 0] Initial document record registered in LanceDB:",
        localDoc,
      );
      ...
      console.log(
        "🔍 [Recall App] [Stage 1] Extracting text content. Server fallback required?",
        mustUseServerFallback,
      );
      ...
      if (!mustUseServerFallback) {
        try {
          console.log(
            "⚙️ [Recall App] [Stage 1] Parsing file locally via Tauri IPC invoke('parse_file')...",
          );
          text = await invoke("parse_file", { path: filePath });

          console.log(
            `✅ [Recall App] [Stage 1] Local file parsing complete. Extracted ${text.length} characters.`,
          );

          console.log(
            `🧩 [Recall App] [Stage 1] Building smart chunks locally...`,
          );
          chunks = buildSmartChunks(text, { filename });

          console.log(
            `📦 [Recall App] [Stage 1] Chunks generated locally: ${chunks.length} chunks.`,
          );
          ...
          console.warn(
            "⚠️ [Recall App] [Stage 1] Local parse/index failed or returned empty content; using stateless server OCR fallback.",
            localErr,
          );
          text = "";
          chunks = [];
        ...
        console.log(
          "🖥️ [Recall App] [Stage 1] File requires server-side handling (e.g. image OCR). Processing on server...",
        );
        metadata = await processFileOnServer(filePath, fileObject);
      }
      ...
      if (metadata.text || metadata.chunks) {
        text = metadata.text || "";
        chunks =
          Array.isArray(metadata.chunks) && metadata.chunks.length > 0
            ? normalizeIndexedChunks(filename, text, metadata.chunks)
            : buildSmartChunks(text, { filename });

        console.log(
          `📦 [Recall App] [Stage 1] Received parsed content stats: character length = ${text.length}, chunk count = ${chunks.length}`,
        );

        if (chunks.length === 0) {
          throw new Error(
            "Server fallback did not return indexable text chunks.",
          );
        }
      }
      ...
      console.log(
        "✍️ [Recall App] [Stage 1] Saving intermediate 'indexed_text' document status...",
      );
      const indexedDoc = await saveLocalDocument({
        ...
        summary:
          metadata.summary ||
          localDoc.summary ||
          buildDocumentSummary(filename, text, chunks),
      ...
      console.log(
        "✨ [Recall App] [Stage 1 Complete] Intermediate metadata & text preview saved:",
        indexedDoc,
      );
      ...
      console.log(
        "⚡ [Recall App] [Stage 2] Starting background vector indexing pipeline. Status set to 'indexing_vectors'...",
      );
      ...
      console.log(
        "🌟 [Recall App] [Stage 3] Completing indexing pipeline. Setting status to 'processed'...",
      );
      ...
      try {
        const detail = await invoke("get_local_document", {
          documentId: newDocId,
        });
      ...
      console.log(
        `🏆 [Recall App] [Stage 3 Complete] Document "${filename}" successfully indexed and processed locally in the background!`,
        processedDoc,
      );
    } catch (err) {
      console.error(
        "🔴 [Recall App] [Ingestion Error] Local document ingestion pipeline failed:",
        err,
      );
      ...
      console.log(
        "🚨 [Recall App] Document status set to 'failed'. Summary updated with error details.",
      );
  ...
  const uploadFile = async (file) => {
    alert(
      `Client-first indexing requires the desktop app. Could not index ${file?.name || "this file"} in browser mode.`,
    );
  };
  ...
  const handleSearch = async (e) => {
      ...
      try {
        ...
        const mappedResults = localResults.map((item) => {
          ...
          const similarity = Math.max(
            0.1,
            Math.min(1.0, 1.0 - item.score / 2.0),
          );
          return {
            id: item.id,
    ...
    alert(
      "Client-first semantic search requires the desktop app and local LanceDB index.",
    );
    setIsSearching(false);
  };
  ...
  const getLocalChatContext = async (query) => {
    ...
    try {
      ...
      console.warn(
        "Failed to build local chat context; sending message without document context.",
        err,
      );
      return [];
    }
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (!currentSessionId) {
        ...
        if (!createRes.ok)
          throw new Error("Failed to create a default chat session");
        const newSession = await createRes.json();
        setChatSessions([newSession]);
        setActiveSessionId(newSession.id);
      ...
      const res = await fetch(
        `${API_BASE}/chat/session/${currentSessionId}/message/`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          >
            ...
            <div>
              ...
              <p style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
                This {doc.file_type || "text"} document has been fully
                OCR-digitized and indexed into 768-dimensional semantic
                embeddings.
              </p>
            </div>
          </div>
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            Dev Mode credentials: <strong>admin</strong> /{" "}
            <strong>admin</strong>
          </div>
        </div>
      </div>
        ...
        >
          ...
          >
            ...
            <div>
              ...
              >
                {modelLoadingError
                  ? "Semantic Engine Download Failed"
              ...
              >
                {modelLoadingError ||
                  "We are configuring the local multilingual BGE-M3 model for fully offline semantic search (~560 MB). This only happens once."}
              </p>
            </div>
            <div
              style={{
            ...
            >
              <div
                style={{
                  ...
                  fontSize: "0.75rem",
                  fontWeight: "700",
                  color: modelLoadingError ? "#ef4444" : "#7c3aed",
                }}
              >
                <span>
                  {modelLoadingError
      ...
      >
        ...
        <div style={{ padding: "1rem" }}>
          <div
            ...
            style={{
              padding: "1.25rem",
              border: `1px dashed ${isDragOver ? "#7c3aed" : "var(--panel-border)"}`,
              borderRadius: "10px",
      ...
      >
        ...
        >
          <form
            ...
            className="search-container-glow"
          ...
          >
            {isLoadingDocDetails ? (
              ...
              >
                ...
                {/* Tags */}
                {selectedDocDetails.tags &&
                  selectedDocDetails.tags.length > 0 && (
                    <div
                      style={{
                        display: "flex",
                        flexWrap: "wrap",
                ...
                {debugMode &&
                  ...
                  selectedDocDetails.chunks.length > 0 && (
                    <div>
                      ...
                      >
                        Debug: Indexed Text Portions (
                        {selectedDocDetails.chunks.length})
                      </h3>
                      <div
                        style={{
                          display: "flex",
              ...
              >
                ...
                <div>
                  ...
                  >
                    Select a document from your library on the left to review
                    its content, premium OCR extraction text, and AI summary.
                  </p>
                </div>
              </div>
            )}
              ...
              {msg.sources &&
                ...
                (() => {
                  ...
                  for (const src of msg.sources) {
                    ...
                    const filename = src.filename || src.suggested_title;

                    // Deduplicate by both document ID and filename to display whole documents only once
                  ...
                  return (
                    ...
                    >
                      {uniqueSources.map((src, sIdx) => {
                        ...
                        // Find matching document in library by ID or filename
                        const matchingDoc =
                          documents.find((d) => docId && d.id === docId) ||
                          documents.find(
                            (d) => filename && d.filename === filename,
                          );

                        const handleSourceClick = () => {
                          const docToSelect = matchingDoc || {
                            id: docId || `local-${Date.now()}`,
                            filename: filename || "Document",
                            file_type: inferFileType(filename || ""),
                          };
                          handleSelectDocument(docToSelect);
                        };

                        const summaryTextRaw = getSummaryText(matchingDoc);
                        const summaryText =
                          summaryTextRaw && summaryTextRaw.length > 160
                            ? summaryTextRaw.slice(0, 160) + "..."
                            : summaryTextRaw || "No summary available.";
                        ...
                        return (
                          ...
                          >
                            <span
                              ...
                              title="Click to open document preview"
                            >
                              <Link2 size={8} />
                              {filename}
                            </span>
