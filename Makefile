...
DEBUG_ENABLED = $(strip $(filter 1 true yes on,$(DEBUG)) $(findstring --debug,$(MAKEFLAGS)))
FRONTEND_DEBUG_ENV = $(if $(DEBUG_ENABLED),VITE_RECALLOS_DEBUG=true,)
DJANGO_DEBUG_ENV = $(if $(DEBUG_ENABLED),DEBUG=True,)
CELERY_LOG_LEVEL = $(if $(DEBUG_ENABLED),debug,info)

.PHONY: install docker docker-debug migrate backend backend-debug frontend frontend-debug app app-debug
...
.backend-run:
	...
	@set -e; \
	$(DJANGO_DEBUG_ENV) $(PYTHON) backend/manage.py runserver 127.0.0.1:8000 & api=$$!; \
	cd backend && $(DJANGO_DEBUG_ENV) ../$(CELERY) -A recallos worker -l $(CELERY_LOG_LEVEL) & worker=$$!; \
	trap 'kill $$api $$worker 2>/dev/null || true' INT TERM EXIT; \
	wait $$api $$worker

backend: migrate .backend-run

backend-debug: DEBUG=1
backend-debug: backend

frontend:
	cd client && $(FRONTEND_DEBUG_ENV) $(NPM) run dev -- --host 127.0.0.1

frontend-debug: DEBUG=1
...
app: install
	...
	$(DJANGO_DEBUG_ENV) $(PYTHON) backend/manage.py runserver 127.0.0.1:8000 & api=$$!; \
	cd backend && $(DJANGO_DEBUG_ENV) ../$(CELERY) -A recallos worker -l $(CELERY_LOG_LEVEL) & worker=$$!; \
	cd client && $(FRONTEND_DEBUG_ENV) $(NPM) run dev -- --host 127.0.0.1 & frontend=$$!; \
